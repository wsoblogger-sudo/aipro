...
const KEYS = {
  accounts: "ai.accounts.v1",
  session: "ai.session.v1",
  toolAccess: "ai.toolAccess.v1",
  billing: "ai.billing.v1",
  preferences: "ai.preferences.v1",
  tickets: "ai.tickets.v1",
  profile: "ai.profile.v1",
  newsletter: "ai.newsletter.v1",
} as const;
...
export function getTickets(): SupportTicket[] {
  if (!isBrowser()) return [];
  return safeParse<SupportTicket[]>(localStorage.getItem(KEYS.tickets)) ?? [];
}

export function addTicket(ticket: SupportTicket): void {
  if (!isBrowser()) return;
  const current = getTickets();
  localStorage.setItem(KEYS.tickets, JSON.stringify([ticket, ...current]));
}

export function updateTicket(next: SupportTicket): void {
  if (!isBrowser()) return;
  const current = getTickets();
  const updated = current.map((t) => (t.id === next.id ? next : t));
  localStorage.setItem(KEYS.tickets, JSON.stringify(updated));
}

export function getProfile(): UserProfile {
  if (!isBrowser()) return {};
  return safeParse<UserProfile>(localStorage.getItem(KEYS.profile)) ?? {};
}
...
export function setNewsletterEmail(email: string): void {
  if (!isBrowser()) return;
  localStorage.setItem(KEYS.newsletter, email);
}

export function getNewsletterEmail(): string | null {
  if (!isBrowser()) return null;
  return localStorage.getItem(KEYS.newsletter);
}
...
export function resetDemoData(): void {
  ...
  localStorage.removeItem(KEYS.newsletter);
}
