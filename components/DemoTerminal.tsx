...
export default function DemoTerminal() {
  ...
  return (
    <div className="glass-surface neon-outline rounded-3xl p-6">
      ...
      <div className="mt-5 rounded-2xl border border-white/10 bg-black/50 p-4 font-mono text-xs text-zinc-200">
        {lines.map((line, idx) => (
          <div key={`${idx}-${line}`}>{`> ${line}`}</div>
        ))}
        {lines.length < LOGS.length ? <div className="text-zinc-500">&nbsp;</div> : null}
      </div>
    </div>
  );
}
